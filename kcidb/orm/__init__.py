...
assert set(argparse.SCHEMA.types) >= \
...
class Source(ABC):
    ...
    def oo_query(self, pattern_set):
        ...
        assert all(isinstance(r, argparse.Pattern) for r in pattern_set)
...
class Prefetcher(Source):
    ...
    def oo_query(self, pattern_set):
        ...
        for obj_type_name, objs in data.items():
            obj_type = argparse.SCHEMA.types[obj_type_name]
            if not obj_type.parents and objs:
                ...
                prefetch_pattern_set |= argparse.Pattern.parse(
                    argparse.Pattern(None, True, obj_type,
...
class Cache(Source):
    ...
    def reset(self):
        ...
        Reset the cache.
        """
        self.type_id_objs = {type_name: {} for type_name in argparse.SCHEMA.types}
    ...
    def _merge_pattern_response(self, pattern, response):
        ...
        assert isinstance(pattern, argparse.Pattern)
        assert LIGHT_ASSERTS or argparse.SCHEMA.is_valid(response)
        ...
        get_id = argparse.SCHEMA.types[type_name].get_id
        get_parent_id = argparse.SCHEMA.types[type_name].get_parent_id
        ...
        for obj in objs:
            ...
            if base_pattern is not None and \
               pattern.child and pattern.obj_id_set is None:
                ...
                parent_child_pattern = argparse.Pattern(
                    argparse.Pattern(None, True, base_type,
        ...
        if pattern.child and base_pattern in self.pattern_responses and \
           pattern.obj_id_set is None:
            parent_get_id = argparse.SCHEMA.types[base_type.name].get_id
        ...
        for child_relation in pattern.obj_type.children.values():
            ...
            sub_pattern = argparse.Pattern(pattern, True, child_relation.child)
            if sub_pattern in self.pattern_responses:
                # For each object in our response
                for obj in objs:
                    # Create pattern for its children of this type
                    parent_child_pattern = argparse.Pattern(
                        argparse.Pattern(None, True, pattern.obj_type, {get_id(obj)}),
